...
exports.updateEnseignant = async (req, res) => {
  try {
    ...
    // Optionally hash password if provided and changed
    ...
    // Only update password in User if provided
    if (password) {
      userUpdateData.password = password; // Let User model handle hashing
    }

    await User.findOneAndUpdate(
      { email: existingEnseignant.email },
      userUpdateData
    );
...
exports.getSubmissionsWithAnswerKey = catchAsyncError(async (req, res) => {
  ...
  const submissions = await Submission.find({ examId }).populate(
    "studentId",
    "name email"
  );
